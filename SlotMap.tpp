...
namespace Ocacho::DataStructures
{
	...
	{
			assert(("Invalid Key to free!", is_valid(p_key)));
	...
	SlotMap<DATA_T, INDEX_T, Capacity>::allocate()
	{
		...
		assert(("Error. Wrong control of freelist_", freelist_ < Capacity));
	...
	{
		assert(("Invalid Key! Trying to access an invalid position...", is_valid(p_key)));
